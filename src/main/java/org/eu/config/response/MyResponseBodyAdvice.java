...
@Slf4j
//@ControllerAdvice(basePackages = "org.jit.sose.springboot_war.controller") // 两种方式
...
//		log.info("MyResponseBodyAdvice==>returnType:" + returnType);
//		log.info("MyResponseBodyAdvice==>returnObj:" + obj);
        CommonResp resp = new CommonResp();
        resp.setCode(ResponseEnum.SUCCESS.code());
        resp.setMsg(ResponseEnum.SUCCESS.msg());
        resp.setObj(obj);

        if(getIgnorePath().contains(request.getURI().getPath())){
            return obj;
        }
     ...
     * @return
     */
    private Set<String> getIgnorePath(){
        Set<String> ignoreSet = new HashSet<String>();
        ignoreSet.add("/battle/video/convertToBase64");
